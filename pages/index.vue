  ...
  <b-container fluid id="app">
    <b-row>
      <b-col>
        ...
        <collapse-section :sectionName="'Add Games'">
          <b-form-group label-cols label="Load from BGG Collection">
            <b-input-group>
              <b-form-input
                id="input-username"
                v-model="username"
          ...
          <b-modal v-model="search.showModal" title="Add a Game">
            <b-form-group label-cols label="Game Name">
              <b-form-input
            ...
            <div v-show="searchingForGame">Loading...</div>
          ...
          <b-form-group v-if="extraGames.length" label="Extra Games" label-cols>
            <b-input-group>
        ...
        <!-- FILTER SECTION -->
        <!--  -->
        <collapse-section :sectionName="'Sort and Filter Games'">
          <div id="filter-switch-container">
            Simple
            <span
              ><b-checkbox switch v-model="preFilter.showAdvanced" size="sm"
                >Advanced</b-checkbox
              ></span
            >
          </div>
          ...
          <div id="advancedFilters" v-else>
            ...
            >
              <DoubleSlider
                v-model="sliderComplexity"
                :options="sliderOptsComplexity"
              />
            </b-form-group>
            ...
            >
              <DoubleSlider
                v-model="sliderNumPlayers"
                :options="sliderOptsNumPlayers"
              />
            </b-form-group>
            ...
            >
              <DoubleSlider
                v-model="sliderPlaytime"
                :options="sliderOptsPlaytime"
              />
            </b-form-group>
    ...
    <b-row>
      <b-col>
        <b-row>
          ...
          <b-col>
            <div v-if="displayGames.length > 1">
              <b-button
                id="btn-random"
                v-if="filter.randomGame == null"
                @click="doRandomGame"
                >Pick A Random Game For Me</b-button
              >
              <b-button v-else @click="filter.randomGame = null"
                >See All Games</b-button
              >
            </div>
            <b-modal
              v-model="randomGameModal"
              ok-only
              title="You should play..."
            >
        ...
        <b-list-group v-if="listView == 'comfortable'">
          <GameEntry
        ...
        >
          ...
          <template #cell(name)="data">
            <div
              ...
              :title="data.value"
            >
              <a target="_blank" :href="getGameURL(data.item.id)">{{
                data.value
              }}</a>
            </div>
          </template>
          ...
          <template #cell(rating)="data">
            {{
              data.value > 0
                ? `${data.value}${
                    settings.showNumVotes.rating
                      ? ` (${data.item.ratingVotes})`
                      : ""
                  }`
                : "-"
            }}
          ...
          <template #cell(complexity)="data">
            {{
              data.value > 0
                ? `${data.value}${
                    settings.showNumVotes.complexity
                      ? ` (${data.item.complexityVotes})`
                      : ""
                  }`
                : "-"
            }}
          ...
          <template #cell(players)="data">
            {{
              data.item.minplayers == data.item.maxplayers
                ? data.item.minplayers
                : data.item.minplayers + " - " + data.item.maxplayers
            }}
          </template>
...
export default {
    ...
    displayGames() {
      ...
      return allGames.map((game) => {
        ...
        for (let i = 0; i < this.filter.tags.length; i++) {
          let matchIdx = game.tags.findIndex((tag) => {
            return tag.id == this.filter.tags[i].id;
          });
          if (matchIdx >= 0) {
  ...
  methods: {
    ...
    filterGames(game, filter) {
      if (filter.complexity[0] > 0 && game.complexity < filter.complexity[0])
        return false;
      if (filter.complexity[1] < 5 && game.complexity > filter.complexity[1])
        return false;

      if (filter.numplayers[0] > 1 && game.minplayers > filter.numplayers[0])
        return false;
      if (filter.numplayers[1] < 12 && game.maxplayers < filter.numplayers[1])
        return false; // TODO: 12 max players is arbitrary

      if (filter.playtime[0] > 0 && game.minplaytime > filter.playtime[0])
        return false;
      if (filter.playtime[1] < 120 && game.maxplaytime < filter.playtime[1])
        return false; // TODO: 120 minutes is arbitrary
    ...
    async getCollection() {
      try {
        ...
        if (!results) {
          // tryGet returned null, probably because cross-origin error, which appears to be that the username didn't exist
          throw [
            "Invalid Username",
            "That does not appear to be a valid username on BoardGameGeek.com",
            "danger",
          ];
        }
        if (results.message) {
        ...
        if (!results.items?.item?.length) {
          throw [
            "No Games",
            `There does not appear to be any games in this user's collection`,
            "info",
          ];
        }
        const gameIds = results.items.item.map((game) => {
        ...
        if (!results) {
          throw [
            "Problem",
            `There was a problem fetching the games in this user's collection`,
            "danger",
          ];
        }
        this.collectionGames = this.mapGameObjects(
          results.items.item,
          this.username
        );
        this.doToast(
          "Games added",
          `Successfully added ${gameIds.length} games from ${this.username}'s collection of owned games.`,
          "success"
        );
    ...
    async gameSearch(query) {
      ...
      if (!parsed.items?.item?.length) {
        this.searchingForGame = false;
        return; // No results
      }
      let games = parsed.items.item.filter(
        (game) => game.name[0].$.type == "primary"
      ); // Filter out translated names
      games = games.map((game) => {
        let existsInLib = this.allGames;
        return {
          id: game.$.id,
          existsInLib,
      ...
      let expansions = games.filter((x) => {
        return x.expansion;
      });
      let basegames = games.filter((x) => {
        return !x.expansion;
      });
      // Remove duplicates
      expansions.map((exp) => {
        let baseIdx = basegames.findIndex((base) => {
          return base.id == exp.id;
        });
        if (baseIdx >= 0) {
          basegames.splice(baseIdx, 1);
        }
      });
      basegames = basegames.sort((a, b) => {
        return b.year - a.year;
      });
      expansions = expansions.sort((a, b) => {
        return b.year - a.year;
      });
      // Set display lists
      ...
      if (basegames.length) {
        ...
        idString = basegames.map((game) => game.id).join(",");
        url = `thing?id=${idString}`;
        parsed = await this.tryGet(url);
        parsed.items.item.map((res) => {
          let foundGame = this.search.basegames.find(
            (game) => game.id == res.$.id
          );
          if (foundGame) {
      ...
      if (expansions.length) {
        ...
        parsed = await this.tryGet(url);
        parsed.items.item.map((res) => {
          let foundGame = this.search.expansions.find(
            (game) => game.id == res.$.id
          );
          if (foundGame) {
            foundGame.thumbnail = res.thumbnail?.[0];
    ...
    doRandomGame() {
      ...
      if (this.filterGames.length >= 2) {
        ...
        do {
          this.randomGame =
            this.filteredGames[
              Math.floor(Math.random() * this.filteredGames.length)
            ];
    ...
    async getExtraGames(gameIds) {
      for (let id of gameIds) {
        ...
        if (parsed.items.item) {
          const newGame = this.mapGameObjects(parsed.items.item, "Extra");
          this.extraGames.push(...newGame);
          this.doToast(
            "Game Added",
            `${newGame[0].name} was added to the list`,
            "success"
          );
        } else {
    ...
    mapGameObjects(uglyGames, source) {
      return uglyGames.map((game) => {
        const tags = game.link
          .filter((link) => {
            return (
        ...
        return {
          ...
          rating: parseFloat(
            game.statistics[0].ratings[0].average[0].$.value
          ).toFixed(1),
    ...
    copyPermalink() {
      try {
        ...
        const encodeParams = {
          ...
          games: this.extraGames.length
            ? this.extraGames.map((game) => game.id).join()
            : null,
        ...
        this.doToast(
          "Copy Failed.",
          "Your browser does not support this operation.",
          "danger"
        );
      }
    },
